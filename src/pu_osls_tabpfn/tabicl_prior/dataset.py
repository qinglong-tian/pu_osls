...
class SCMPrior(Prior):
    ...
    def generate_dataset(self, params: Dict[str, Any]) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        if params["prior_type"] == "mlp_scm":
            ...
            try:
                ...
                raise ImportError(
                    "tree_scm prior requires optional dependency 'xgboost'. "
                    "Install xgboost or use prior_type='mlp_scm'."
                ) from exc
            prior_cls = TreeSCM
        else:
    ...
    def get_prior(self) -> str:
        ...
        if self.prior_type == "mix_scm":
            mix_probs = self.fixed_hp.get("mix_probs", self.fixed_hp.get("mix_probas", [0.7, 0.3]))
            return np.random.choice(["mlp_scm", "tree_scm"], p=mix_probs)
        else:
